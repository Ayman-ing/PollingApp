...
public class PollService {
    ...
    public PagedResponse<PollResponse> getAllPolls(CustomUserDetails currentUser,
        ...
        List<Long> pollIds = polls.map(PollEntity::getId).getContent();
        Map<Long, Long> choiceVoteCountMap = getChoiceVoteCountMap(pollIds);
        Map<Long, Long> pollUserVoteMap = getPollUserVoteMap(currentUser, pollIds);
        Map<Long, UserEntity> creatorMap = getPollCreatorMap(polls.getContent());
    ...
    public PagedResponse<PollResponse> getPollsCreatedBy(String username, CustomUserDetails currentUser, int page, int size) {
        ...
        Pageable pageable = PageRequest.of(page, size, Sort.Direction.DESC, "createdAt");
        Page<PollEntity> polls = pollRepository.findByCreatedBy(user.getId(), pageable);
    ...
    public PagedResponse<PollResponse> getPollsVotedBy(String username, CustomUserDetails currentUser, int page, int size) {
        ...
        List<PollEntity> polls = pollRepository.findByIdIn(pollIds, sort);

        // Map Polls to PollResponses containing vote counts and poll creator details
        Map<Long, Long> choiceVoteCountMap = getChoiceVoteCountMap(pollIds);
        Map<Long, Long> pollUserVoteMap = getPollUserVoteMap(currentUser, pollIds);
        Map<Long, UserEntity> creatorMap = getPollCreatorMap(polls);
    ...
    Map<Long, UserEntity> getPollCreatorMap(List<PollEntity> polls) {
        // Get Poll Creator details of the given list of polls
        List<Long> creatorIds = polls.stream()
                .map(PollEntity::getCreatedBy)
                .distinct()
                .collect(Collectors.toList());

        List<UserEntity> creators = userRepository.findByIdIn(creatorIds);
    ...
    public PollEntity createPoll(PollRequest pollRequest) {
        ...
        pollRequest.getChoices().forEach(choiceRequest -> poll.addChoice(new ChoiceEntity(choiceRequest.getText())));
    ...
    public PollResponse castVoteAndGetUpdatedPoll(Long pollId, VoteRequest voteRequest, CustomUserDetails currentUser) {
        ...
        UserEntity user = userRepository.getOne(currentUser.getId());

        ChoiceEntity selectedChoice = poll.getChoices().stream()
